...
class User:
	...
	def SSLHandShake(self, sock):
		...
		print('[ Server]', ret, 'sessionID:', hex(self.__MAC.sessionID))
		print("[ Client] Passed Phase 1")
		...
		ret, MAC = recvMsg(sock)
		if not self.checkMAC(ret, MAC):
			print('[ Client] Failed Phase 2!')
			return False
		e,n = ret.split()
		print("[ Client] Passed Phase 2")
		...
		ret, MAC = recvMsg(sock)
		if not self.checkMAC(ret, MAC):
			print('[ Client] Failed Phase 3!')
			return False
		print("[ Client] Passed Phase 3")
		...
		if msg != 'serverPhase4' or not self.checkMAC(cipher, MAC):
			print('[ Client] That is not the server!')
			return False

		print("[ Client] Passed Phase 4")
		return True
	...
	def run(self):
		...
		#server_address = ('3.125.209.94', 19504)
		print('[Client] Connecting To:', server_address[0], "Port:", server_address[1])
		...
		if successful:
			# p ->start
			# e -> Exit
			print('[Client] Handshake Protocol Succeeded. Connection to  Server Accepted.')
			while (True):
				req = input('[Client] Enter  Info as follows "Key:Value": ')
				...
				if req == "e":
					...
					if not self.checkMAC(cipher, MAC):
						print('[Client] Invalid MAC')
						break
					print('[Server]', msg)
		else:
			sock.close()
			print('[Client] Handshake Protocol Failed. Denied Connection...')
			return
		sock.close()
		print('[Client]  Operations Successful. Exiting...')

	def start(self):
		print('[ Client] Started...')
		while (True):
			op = input('[Client] Type e to Exit, p to Proceed: ')
			if (op == 'e'):
				print('[Client] Shutting off...')
				break
			elif (op == 'p'):
